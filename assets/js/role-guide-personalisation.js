(()=>{
  ...
  style.textContent=`
    ...
    @keyframes wsGuideStepIn{from{opacity:.35;transform:translateY(5px)}to{opacity:1;transform:none}}

    .ws-g2-onboard{position:relative;padding:1px 1px 2px;color:#302638}
    .ws-g2-onboard.ws-g2-step-in{animation:wsGuideStepIn .18s cubic-bezier(.2,.76,.2,1) both}
    ...
    .ws-g2-on-steps i{display:block;width:22px;height:4px;border-radius:99px;background:rgba(124,58,237,.13);transition:width .18s ease,background .18s ease}
    ...
    .ws-g2-on-next{height:44px;min-width:102px;padding:0 16px;border:0;border-radius:13px;background:#7c3aed;color:#fff;font:800 12px/1 Inter,sans-serif;cursor:pointer;box-shadow:0 8px 18px rgba(124,58,237,.18);transition:transform .15s ease,opacity .15s ease,box-shadow .15s ease}
    .ws-g2-on-next:active:not(:disabled){transform:scale(.98)}
    ...
    .ws-g2-on-role{display:flex;align-items:center;gap:10px;min-height:62px;padding:8px 11px;border:0;border-radius:17px;background:rgba(255,255,255,.54);color:#473c50;cursor:pointer;text-align:left;box-shadow:0 8px 22px rgba(55,38,78,.055);transition:transform .15s ease,background .15s ease,box-shadow .15s ease}
    .ws-g2-on-role:active{transform:scale(.985)}
    ...
    @media(prefers-reduced-motion:reduce){.ws-g2.fresh .ws-g2-ring img,.ws-g2.fresh .ws-g2-ring:after,.ws-g2-onboard.ws-g2-step-in{animation:none!important}.ws-g2-name-shell,.ws-g2-sheet.ws-g2-keyboard-aware,.ws-g2-on-next,.ws-g2-on-role{transition:none!important}}
  `;
  ...
  const renderOnboarding=()=>{
    ...
    if(step===1){
      ...
      input?.addEventListener('keydown',e=>{
        if(e.key==='Enter'&&!next?.disabled)advance(e);
      });
