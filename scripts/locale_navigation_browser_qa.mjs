...
async function waitForShell(page,locale,isEvent=false){
  ...
  // Give async shared shell helpers time to finish. This catches late duplicate nav
  // injection rather than only checking the first frame after translation.
...
async function assertNavigation(locale,path,isEvent=false){
  ...
  try{
    ...
    const state=await page.evaluate(()=>{
      ...
      return{
        ...
        registrationFields:{
          ...
          organisationType:field('organisation_type'),
    ...
    if(isEvent){
      ...
      await page.locator('#eventRegistrationForm').scrollIntoViewIfNeeded();
      ...
      const email=page.locator('#eventRegistrationForm [name="email"]');
      if(await email.count()){
        await email.fill('qa-locale@example.com');
        if(await email.inputValue()!=='qa-locale@example.com') failures.push(`[${locale}/event] translated registration form is not interactive`);
      }
    }
